...
from agent_glm import UnlearningAgent
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="A CLI for the Unlearning Agent using GLM-4.6 (Zhipu AI) to process a query while removing "
                    "specified subjects.",
    ...
    model_group.add_argument(
        ...
        help="Zhipu AI API Key. Can also be set via ZHIPU_API_KEY environment variable."
    )
    model_group.add_argument(
        "--hf-token",
        type=str,
    ...
    if not args.zhipu_api_key:
        logger.critical(
            "FATAL: Zhipu AI API key not found. Please provide it via --zhipu-api-key or set ZHIPU_API_KEY "
            "environment variable.")
        return 1

    if args.hf_check_model and not args.hf_token:
        logger.warning(
            "A Hugging Face check model is specified, but no HF token is provided. This may fail for gated or private "
            "models.")
